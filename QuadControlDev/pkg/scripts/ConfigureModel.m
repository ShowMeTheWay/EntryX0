...
if(exist([sys, '_ert_rtw/', sys,'.c']) == 2)
    copyfile([sys, '_ert_rtw/', sys,'.c'],'../src');
...
if(exist([sys, '_ert_rtw/', sys,'.h']) == 2)
    copyfile([sys, '_ert_rtw/', sys,'.h'],'../src/');
...
if(exist([sys, '_ert_rtw/', sys,'_data.c']) == 2)
    copyfile([sys, '_ert_rtw/', sys,'_data.c'],'../src');
    ...
    if(exist(['../src/', sys,'_data.c']) == 2)
        delete(['../src', sys,'_data.c']);
...
if(exist([sys, '_ert_rtw/', sys,'_private.h']) == 2)
    copyfile([sys, '_ert_rtw/', sys,'_private.h'],'../src');
    delete([sys, '_ert_rtw/', sys,'_private.h']);
    disp([sys,'_private.h - successfully copied']);
else
    %Check if from former code generation file was available
    if(exist(['../src/', sys,'_private.h']) == 2)
        delete(['../src', sys,'_private.h']);
...
if(exist([sys, '_ert_rtw/', sys,'_types.h']) == 2)
    copyfile([sys, '_ert_rtw/', sys,'_types.h'],'../src');
    delete([sys, '_ert_rtw/', sys,'_types.h']);
    disp([sys,'_types.h - successfully copied']);
else
    %Check if from former code generation file was available
    if(exist(['../src/', sys,'_types.h']) == 2)
        delete(['../src/', sys,'_types.h']);
...
if(exist([sys, '_ert_rtw/', 'rtw','types.h']) == 2)
    copyfile([sys, '_ert_rtw/', 'rtw','types.h'],'../src');
    delete([sys, '_ert_rtw/', 'rtw','types.h']);
    disp(['rtw','types.h - successfully copied']);
else
    %Check if from former code generation file was available
    if(exist(['../src/', 'rtw','types.h']) == 2)
        delete(['../src/', 'rtw','types.h']);
...
rmpath('../');
disp('All Files have been generated and successfully copied to ../src.');
end
